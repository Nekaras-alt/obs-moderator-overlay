...
Packed Electron builds call `checkForUpdates` on ready (log + tray balloon only; no auto-download). If `electron-updater` is missing from `node_modules`, startup skips the check with a console log. Publish releases with electron-builder + a GitHub/generic feed for real updates.
...
| `/no-install` | не копировать плагин в `%AppData%\obs-studio\plugins` |
...
Перезапустите OBS → источники **OMO Overlay (Native)** / **(Browser)**, dock **OMO Connector**.
